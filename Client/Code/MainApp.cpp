...
HRESULT CMainApp::SetUp_Setting(LPDIRECT3DDEVICE9* ppGraphicDev)
{
	...
	FAILED_CHECK_RETURN(Engine::Ready_Font(m_pGraphicDev, L"나눔바른고딕", L"Button_Make", 12, 15, FW_THIN), E_FAIL);

	//Mouse
	FAILED_CHECK_RETURN(Engine::Ready_Font((*ppGraphicDev), L"Mapo배낭여행", L"Mouse_Title", 15, 22, FW_HEAVY), E_FAIL);
	FAILED_CHECK_RETURN(Engine::Ready_Font((*ppGraphicDev), L"Mapo배낭여행", L"Mouse_Sub", 10, 20, FW_HEAVY), E_FAIL);
