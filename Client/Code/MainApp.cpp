...
HRESULT CMainApp::SetUp_Setting(LPDIRECT3DDEVICE9* ppGraphicDev)
{
	...
	//UI
	FAILED_CHECK_RETURN(Engine::Ready_Font(m_pGraphicDev, L"Mapo배낭여행",  L"Panel_Title", 15, 15, FW_THIN), E_FAIL);
	FAILED_CHECK_RETURN(Engine::Ready_Font(m_pGraphicDev, L"Mapo배낭여행", L"Panel_Info",  6, 10, FW_THIN), E_FAIL);
	FAILED_CHECK_RETURN(Engine::Ready_Font(m_pGraphicDev, L"Mapo배낭여행", L"Button_Make", 10, 15, FW_THIN), E_FAIL);
