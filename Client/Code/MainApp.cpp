...
HRESULT CMainApp::SetUp_Setting(LPDIRECT3DDEVICE9* ppGraphicDev)
{
	...
	//UI
	FAILED_CHECK_RETURN(Engine::Ready_Font(m_pGraphicDev, L"나눔고딕코딩",  L"Panel_Title", 10, 22, FW_THIN), E_FAIL);
	FAILED_CHECK_RETURN(Engine::Ready_Font(m_pGraphicDev, L"나눔고딕코딩", L"Panel_Info",  5, 12, FW_THIN), E_FAIL);
	FAILED_CHECK_RETURN(Engine::Ready_Font(m_pGraphicDev, L"나눔고딕코딩", L"Button_Make", 8, 12, FW_THIN), E_FAIL);

	//Mouse
	FAILED_CHECK_RETURN(Engine::Ready_Font((*ppGraphicDev), L"Mapo배낭여행", L"Mouse_Title", 15, 22, FW_HEAVY), E_FAIL);
	FAILED_CHECK_RETURN(Engine::Ready_Font((*ppGraphicDev), L"Mapo배낭여행", L"Mouse_Sub", 6, 20, FW_HEAVY), E_FAIL);
