...
//			{
//				const vector<CGameObject*>& arrItem = scenemgr::Get_CurScene()->GetGroupObject(eLAYER_TYPE::GAME_LOGIC, eOBJECT_GROUPTYPE::ITEM);
//				for (auto iter : arrItem)
//				{
//					if (iter->GetObjName() == m_pUI_Name)
//					{
//						m_pTextureCom[i] = dynamic_cast<Engine::CTexture*>(iter->Find_Component(ID_STATIC, L"proto_texture"));
//						break;
//					}
//				}
//
...
//		return itemCount = 0;
//	else
//		return itemCount = m_mapItem.find(_ItemName)->second;//아이템이 있으면 해당 아이템 개수 반환
//}
//
...
//	//아이템 카운트 초기화
//	itemCount = 0;
//	//아이템이 들어오면 해당 칸에 슬롯을 true로 만들고 다음 아이템이 들어왔어도 첫 번째 칸에 아이템이 있으니 다음칸을 true;
...
//{
//
//	//아이템슬롯에 칸에 넣기 전에 해당 아이템이 몇 개 있는지 확인
//	CallFind_ItemCount([&]() {
//		itemCount = Find_ItemCount(_ItemName); }, _ItemName);
//
//	//만약 아이템 칸에 아이템이 없으면 해당 칸에 아이템을 넣음
//	if (itemCount <= 0)
//	{
...
//		return;
//	}
//	//아이템 칸에 아이템 있으면 해당 칸에 있는 아이템에 개수를 더함
//	else
//	{
//		itemCount = itemCount + _ItmeCount;
//		m_pUI_Name = _ItemName;
